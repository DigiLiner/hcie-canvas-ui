 ...
 */
export function applyBoxBlur(imageData, radius = 3) {
    if (radius <= 0)
        return imageData;
 ...
 */
export function applyMelt(imageData, amount = 30) {
    if (amount <= 0)
        return imageData;
    const width = imageData.width;
    const height = imageData.height;
 ...
 */
export function applyShear(imageData, amount = 40, horizontal = true, direction = 1) {
    const width = imageData.width;
    const height = imageData.height;
    const src = imageData.data;
    const dst = new Uint8ClampedArray(src.length);
 ...
 */
export function applyMosaic(imageData, blockSize = 10) {
    if (blockSize <= 1)
        return imageData;
    const width = imageData.width;
    const height = imageData.height;
...
window.applyNegative = applyNegative;
window.applyGrayscale = applyGrayscale;
window.applyBoxBlur = applyBoxBlur;
window.applyMelt = applyMelt;
window.applyShear = applyShear;
window.applyMosaic = applyMosaic;
