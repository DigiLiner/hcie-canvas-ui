...
import { DrawAction } from '@hcie/core';
import { restoreSettingsForTool } from '@hcie/shared';
import { renderLayers, getActiveLayer, updateLayerPanel } from './layers';
import { drawShapesToCtx } from '@hcie/shared';
import { tooltipManager } from '@hcie/shared';
import { drawSpray } from '@hcie/tools';
import { floodFill } from '@hcie/tools';
import { drawPen, drawLine, drawRect, drawCircle, drawBrush, drawEraser, drawRoundedRect, drawEllipse } from '@hcie/tools';
import { magicWandSelection, buildRectSelection, buildEllipseSelection, buildLassoSelection, isPointInSelection, moveSelectionMask, moveSelectionContent, drawSelectionBorder, getRectBorder, getEllipseBorder } from '@hcie/tools';
import { convertPsdToLayers } from '@hcie/io';
"use strict";
//Canvas container
...
export let originalCanvas = document.getElementById("originalCanvas");
 ...
 */
export function parseColorToRgba(colorStr, opacity = 1.0) {
...
/////////////////   D  O  W  N /////////////////////
////////////////////////////////////////////////////
export function on_canvas_mouse_down(e) {
    if (!can)
        return;
    ...
    switch (g.current_tool.id) {
        case Tool.Wand.id:
            ...
            if (g.wand_all_layers && originalCanvas) {
                // Pick from composite
                sourceCtx = originalCanvas.getContext('2d', { willReadFrequently: true });
            }
            else {
                // Pick from active layer
                const activeLayer = getActiveLayer();
                if (activeLayer) {
                    sourceCtx = activeLayer.canvas.getContext('2d', { willReadFrequently: true });
                }
            }
        ...
        case Tool.PolySelect.id:
            ...
            if (g.isSelectionActive && !hasDraftPoints && typeof isPointInSelection === 'function' && isPointInSelection(g.pX, g.pY)) {
                ...
                g.movingSelection = false;
                // IF we are starting a NEW drafting session, we should clear the OLD selection.
                if (!hasDraftPoints) {
                    if (g.isSelectionActive) {
                ...
                // We keep g.drawing = true to allow mouse_move to show the rubber-band
            }
            break;
        case Tool.Flood_Fill.id:
            ...
            if (g.fill_all_layers && originalCanvas) {
                ...
                if ((checkLayer.type === 'vector' || (checkLayer.shapes && checkLayer.shapes.length > 0))) {
                    ...
                    // Draw raster part
                    compositionCtx.drawImage(fillSourceCanvas, 0, 0);
                    // Draw vector part
            ...
            const fillSourceCtx = fillSourceCanvas.getContext('2d', { willReadFrequently: true });
            if (!fillSourceCtx) {
            ...
            // Pass the selection mask data if active for proper constraint
            const maskData = (g.isSelectionActive && g.selectionCanvas)
                ? g.selectionCanvas.getContext('2d', { willReadFrequently: true }).getImageData(0, 0, g.image_width, g.image_height).data
                : null;
            floodFill(g.startX, g.startY, checkLayer.ctx, fillColor, toleranceObj, fillSourceCtx, maskData);
            finishDrawing(e);
...
/////////////////   M  O  V  E /////////////////////
////////////////////////////////////////////////////
export function on_canvas_mouse_move(e) {
    if (!can)
        return;
    const rect = can.getBoundingClientRect();
    const x = (e.clientX - rect.left) / g.zoom;
    ...
    if (mousePosition)
        mousePosition.textContent = `X: ${g.pX}, Y: ${g.pY}`;
    if (statusMessage)
        statusMessage.innerHTML = g.drawing ? "Drawing" : "Ready";
    ...
    if (isVectorShapeTool && isVectorLayer) {
        ...
        if (g.movingSelection) {
            ...
            if (typeof moveSelectionMask === 'function')
        ...
        if (g.movingSelection) {
            ...
            const points = [...window.polyPoints, { x: g.pX, y: g.pY }];
            // Also close it to the start point if length > 2
            const closedPath = points.length > 2 ? [...points, window.polyPoints[0]] : points;
            g.selectionPreviewBorder = [closedPath];
            // Explicitly draw on tempCtx for live feedback
            if (tempCtx) {
                const isDark = document.body.getAttribute('data-theme') === 'dark';
                tempCtx.clearRect(0, 0, tempCtx.canvas.width, tempCtx.canvas.height);
                tempCtx.beginPath();
                tempCtx.moveTo(window.polyPoints[0].x, window.polyPoints[0].y);
                for (let i = 1; i < window.polyPoints.length; i++) {
                    const ctx = tempCtx;
                    ctx.lineTo(window.polyPoints[i].x, window.polyPoints[i].y);
                }
                // Line to current mouse
                tempCtx.lineTo(g.pX, g.pY);
                // Dash it if it's the "closing line" or the rubber band
                tempCtx.setLineDash([5, 5]);
                tempCtx.strokeStyle = isDark ? '#fff' : '#000';
                tempCtx.lineWidth = 1;
                tempCtx.stroke();
                tempCtx.setLineDash([]);
                // Draw nodes
        ...
        if (g.current_tool.id === Tool.MoveSelection.id) {
            ...
            if (typeof moveSelectionContent === 'function')
...
export function on_canvas_mouse_up(_e) {
    if (can)
 ...
 */
export function applySelectionMaskToCtx(targetCtx) {
    if (g.isSelectionActive && g.selectionCanvas) {
        targetCtx.save();
        targetCtx.globalAlpha = 1.0;
        targetCtx.globalCompositeOperation = 'destination-in';
        targetCtx.drawImage(g.selectionCanvas, 0, 0);
        targetCtx.restore();
    }
}
export function finishDrawing(e) {
    ...
    if (activeLayer) {
        ...
        if (tempCtx)
            applySelectionMaskToCtx(tempCtx);
        ...
        try {
            ...
            if (isVectorShapeTool && isVectorLayer) {
                ...
                // Eraser now draws its MASK on tempCanvas. We subtract it from the layer.
                if (tempCanvas) {
                    activeLayer.ctx.save();
                    activeLayer.ctx.globalCompositeOperation = 'destination-out';
                    activeLayer.ctx.drawImage(tempCanvas, 0, 0);
                    activeLayer.ctx.restore();
                    console.log(`[DEBUG] Subtracted eraser from layer: ${activeLayer.name}`);
                }
            }
            else {
...
export function selectTool(tool) {
    if (!tool)
        return;
    g.current_tool = tool;
    restoreSettingsForTool(tool.id, g);
    ...
    if (!isVectorTool) {
        if (typeof window.vectorToolManager !== 'undefined') {
            window.vectorToolManager.cancelEdit();
        }
    }
    renderLayers();
    ...
    // Cleanup drafting points if switching tools
    if (window.polyPoints)
        window.polyPoints = [];
    if (window.lassoPoints)
        window.lassoPoints = [];
...
export function restoreUndoState() {
    ...
    const oCtx = originalCanvas?.getContext("2d") ?? null;
    if (oCtx)
...
export function applyZoom(oldZoom, e) {
    ...
    if (zoomDisplay)
        zoomDisplay.textContent = ` Zoom: ${Math.round(g.zoom * 100)}%`;
...
if (can) {
    ...
    can.addEventListener('dblclick', (e) => {
        if (g.current_tool.id === Tool.PolySelect.id && window.polyPoints && window.polyPoints.length > 2) {
            try {
                // Remove the last point if it was accidentally added by the double-click sequence
                // Some browsers might trigger mouseup/mousedown before dblclick
                let finalPoints = [...window.polyPoints];
                ...
                if (typeof buildPolygonalSelection === 'function') {
                    buildPolygonalSelection(finalPoints, mode);
                }
                console.log("[DEBUG] Polygon Selection finalized via Double-Click");
            }
            catch (err) {
    ...
    if (e.key === 'Escape') {
        ...
        switch (e.key.toLowerCase()) {
            case 'c':
                ...
                break;
            case 'v':
                if (typeof pasteSelection === 'function')
                    pasteSelection();
                e.preventDefault();
                break;
            case 'x':
