...
import { floodFill, FillTolerance } from '@hcie/tools';
import { 
...
import type { SelectionMode } from '@hcie/shared';
    ...
    switch (g.current_tool.id) {
        ...
        case Tool.Flood_Fill.id:
            ...
            const maskData = (g.isSelectionActive && g.selectionCanvas) 
                ? (g.selectionCanvas as HTMLCanvasElement).getContext('2d', { willReadFrequently: true })!.getImageData(0, 0, g.image_width, g.image_height).data 
                : null;
            floodFill(g.startX, g.startY, checkLayer.ctx as CanvasRenderingContext2D, fillColor, toleranceObj, fillSourceCtx as CanvasRenderingContext2D, maskData);
